/*
Evergreen REST v2 API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 3
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the RouteBuildChangeStatusHandler type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RouteBuildChangeStatusHandler{}

// RouteBuildChangeStatusHandler struct for RouteBuildChangeStatusHandler
type RouteBuildChangeStatusHandler struct {
	Activated *bool `json:"activated,omitempty"`
	Priority *int32 `json:"priority,omitempty"`
}

// NewRouteBuildChangeStatusHandler instantiates a new RouteBuildChangeStatusHandler object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRouteBuildChangeStatusHandler() *RouteBuildChangeStatusHandler {
	this := RouteBuildChangeStatusHandler{}
	return &this
}

// NewRouteBuildChangeStatusHandlerWithDefaults instantiates a new RouteBuildChangeStatusHandler object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRouteBuildChangeStatusHandlerWithDefaults() *RouteBuildChangeStatusHandler {
	this := RouteBuildChangeStatusHandler{}
	return &this
}

// GetActivated returns the Activated field value if set, zero value otherwise.
func (o *RouteBuildChangeStatusHandler) GetActivated() bool {
	if o == nil || IsNil(o.Activated) {
		var ret bool
		return ret
	}
	return *o.Activated
}

// GetActivatedOk returns a tuple with the Activated field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RouteBuildChangeStatusHandler) GetActivatedOk() (*bool, bool) {
	if o == nil || IsNil(o.Activated) {
		return nil, false
	}
	return o.Activated, true
}

// HasActivated returns a boolean if a field has been set.
func (o *RouteBuildChangeStatusHandler) HasActivated() bool {
	if o != nil && !IsNil(o.Activated) {
		return true
	}

	return false
}

// SetActivated gets a reference to the given bool and assigns it to the Activated field.
func (o *RouteBuildChangeStatusHandler) SetActivated(v bool) {
	o.Activated = &v
}

// GetPriority returns the Priority field value if set, zero value otherwise.
func (o *RouteBuildChangeStatusHandler) GetPriority() int32 {
	if o == nil || IsNil(o.Priority) {
		var ret int32
		return ret
	}
	return *o.Priority
}

// GetPriorityOk returns a tuple with the Priority field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RouteBuildChangeStatusHandler) GetPriorityOk() (*int32, bool) {
	if o == nil || IsNil(o.Priority) {
		return nil, false
	}
	return o.Priority, true
}

// HasPriority returns a boolean if a field has been set.
func (o *RouteBuildChangeStatusHandler) HasPriority() bool {
	if o != nil && !IsNil(o.Priority) {
		return true
	}

	return false
}

// SetPriority gets a reference to the given int32 and assigns it to the Priority field.
func (o *RouteBuildChangeStatusHandler) SetPriority(v int32) {
	o.Priority = &v
}

func (o RouteBuildChangeStatusHandler) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RouteBuildChangeStatusHandler) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Activated) {
		toSerialize["activated"] = o.Activated
	}
	if !IsNil(o.Priority) {
		toSerialize["priority"] = o.Priority
	}
	return toSerialize, nil
}

type NullableRouteBuildChangeStatusHandler struct {
	value *RouteBuildChangeStatusHandler
	isSet bool
}

func (v NullableRouteBuildChangeStatusHandler) Get() *RouteBuildChangeStatusHandler {
	return v.value
}

func (v *NullableRouteBuildChangeStatusHandler) Set(val *RouteBuildChangeStatusHandler) {
	v.value = val
	v.isSet = true
}

func (v NullableRouteBuildChangeStatusHandler) IsSet() bool {
	return v.isSet
}

func (v *NullableRouteBuildChangeStatusHandler) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRouteBuildChangeStatusHandler(val *RouteBuildChangeStatusHandler) *NullableRouteBuildChangeStatusHandler {
	return &NullableRouteBuildChangeStatusHandler{value: val, isSet: true}
}

func (v NullableRouteBuildChangeStatusHandler) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRouteBuildChangeStatusHandler) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


